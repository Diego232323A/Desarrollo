...
@app.get("/")
def bienevenida():
    return "Bievenidos aqui podra encontrar diferentes eventos, exhibiciones, festivales y diferentes actividades gracias por pasar :D"

@app.get("/contacto")
def contacto():
    return "GitHub: Marangeles19 , Mail: angela1990j@gmail.com"
...
def get_score_count(platform: str, scored: float, release_year: int):
    ...
    filtro = df[
        (df.platform == platform) & (df.score > scored) & (df.release_year == release_year) & (df.type == 'movie')]
